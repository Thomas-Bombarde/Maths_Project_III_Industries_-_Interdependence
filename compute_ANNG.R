...
WIOT <- functions(periods, i){
  "Extract the world input-output table for a given year"
  WIOT <- periods[[i]]
  X1 <- WIOT[(6:1441),] |> select((5:1440)) |> as.matrix()
  X1 <- apply(X1, 2, as.numeric)
  return(X1)
}
Ai <- functino(WIOT){
  "Return the row-normalised adjacency matrix for a given WIOT"
  for (i in (1:ncol(A))){
    if(sum(X1[i,]) != 0){
      A[i,] <- X1[i,]/sum(X1[i,])
    }
...
adjmat <- 
  function(X1){
  "Compute the adjacency matrix with weights defined by the share of all inputs"
  A <- X1
  for (i in (1:ncol(A))){
    if(sum(X1[i,]) != 0){
      A[i,] <- X1[i,]/sum(X1[i,])
...
adjmat_xtra <- 
  function(X1){
    "Compute the adjacency matrix with weights defined by the share of all 
    extra-industry inputs as in Pichler, Anton and Farmer (2018)"
    A <- X1
    for (i in (1:ncol(A))){
...
prodimprov <- 
  ...
  #input_changes <- diag(t(input_diffhat)%*%A)
  ...
  for (i in (1:n)){
    ...
    for (j in (1:n)){
      ...
      if(is.infinite(ctry_innov[j,k])!=TRUE & is.na(ctry_innov[j,k])!=TRUE){
        tmp <- tmp + as.numeric(Abis[j,i])*ctry_innov[j,k] #problem with Inf
      }
    }
    pred[i] <- tmp
  }
...
setwd("~\\2023 Math Project III\\output")
saveRDS(tables_innov, file = "tables_innov_xtra.Rds")
saveRDS(tables_pred_innov, file = "tables_pred_innov_xtra.Rds")
...
tables_innov <- readRDS(
  "~\\2023 Math Project III\\output\\tables_innov.Rds")
tables_pred <- readRDS(
  "~\\2023 Math Project III\\output\\tables_pred_innov.Rds")
y <- 1995+10
